...
function MainScene:onEnter() -- MainScene 加载执行
		...
		local tick = function()
			if self.stage == "running" then
				...
				local headX , headY = self.snake:GetHeadGrid() -- 取出蛇头
				-- local headX , headY = self.snake:GetHeadGrid()
        
        local b, index = self.blocks:Hit(headX,headY)
        
        if self.fence:CheckCollide(headX,headY) -- 蛇与围墙相撞 或 自杀
...
-- 按键事件处理（用P键 和 R键 来控制正在运行游戏的暂停和恢复）（直接调用了Director的pause()，resume()方法）
